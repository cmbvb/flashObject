		...
		protected function changeClip():void {
			if (_url) {
				if (ResManager.getInstance().hasClass(_url)) {
					loadComplete(_url, false, ResManager.getInstance().getBitmapData(_url));
				} else {
					var fullUrl:String = ResManager.formatExternalImageUrl(_url);
					ResManager.getInstance().load(fullUrl, fullUrl, fullUrl, new Method(loadComplete, [_url, true]));
//					ResManager.getInstance().load(_url, _url, _url, new Method(loadComplete, [_url]));
				}
			}
		}
...
//			TimerManager.getInstance().doLoop(StageUtil.baseFramesToRealFrames(StageManager.getInstance().interval), loop);
		...
		public static function fromPool():Clip {
			return clipPool.length != 0 ? clipPool.pop() : new Clip();
		}
