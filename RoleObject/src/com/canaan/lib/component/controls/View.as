package com.canaan.lib.component.controls
{
	...
	import com.canaan.lib.component.extend.TimerLabel;
	import com.canaan.lib.component.extend.TimerProgressBar;
	...
	{
		public static var uiClassMap:Object = {
		...
		protected function loadProloadFiles():void {
			...
			if (preloadFiles != null && preloadFiles.length != 0 && !preLoadFinished) {
				ResManager.getInstance().loadAssets(preloadFiles, new Method(preloadFilesComplete));
			} else {
		...
		protected function loadUI(path:String):void {
			...
			if (xml != null) {
				...
				ResManager.getInstance().load(url, path, "", new Method(loadUIComplete, [path]), null, true, 0);
			}
		}
		...
		public function reCreate(comp:UIComponent = null):void {
			...
			if (comp is Box) {
				Box(comp).removeAllChildren(true);
			}
