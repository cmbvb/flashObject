...
package com.canaan.lib.component.controls {
	...
	/**单元格渲染时触发*/
	[Event(name="listRender",type="com.canaan.lib.events.UIEvent")]
	...
	public class List extends Box implements IItem {
		...
		protected var _renderHandler:Method;
		protected var _mouseHandler:Method;
		...
		protected var _selectShow:Boolean = true;
		protected var _isVerticalLayout:Boolean = true;
		protected var _cellSize:Number = 20;
		protected var _scrollBarShowForever:Boolean;
		protected var _renderShowForever:Boolean;
			...
			if (_itemRender) {
				...
				for (var k:int = 0; k < numY; k++) {
					for (var l:int = 0; l < numX; l++) {
						...
						addCell(cell);
						if (guideId != null) {
							cell.guideId = guideId + "_" + (k * _repeatY + l);
						}
					}
				}
				if (_scrollBar) {
					...
					var contentWidth:Number = _width || cellWidth * _repeatX - spaceX;
					var contentHeight:Number = _height || cellHeight * _repeatY - spaceY;
			...
			if (e.type == MouseEvent.CLICK || e.type == MouseEvent.ROLL_OVER || e.type == MouseEvent.ROLL_OUT) {
				...
				if (e.type == MouseEvent.CLICK) {
					...
					if (_clickHandler != null) {
						_clickHandler.applyWith([index]);
					}
				} else if (_selectedIndex != index || _selectShow == false) {
					changeCellState(cell, e.type == MouseEvent.ROLL_OVER, 0);
				}
			}
			if (_mouseHandler != null) {
				_mouseHandler.applyWith([e, index]);
			}
		}
		
		protected function changeCellState(cell:Box, visable:Boolean, frame:int):void {
			if (cell.hasOwnProperty("selected") && frame == 1) {
				Object(cell).selected = visable;
			}
			var selectBox:DisplayObject = cell.getChildByName("selectBox");
			if (selectBox) {
				if (selectBox is Clip) {
					selectBox.visible = visable;
					Clip(selectBox).frame = frame;
				} else {
					selectBox.visible = visable && frame == 0;
				}
			}
		}
		...
		/**是否显示选中，默认为true*/
		public function get selectShow():Boolean {
			return _selectShow;
		}
		
		public function set selectShow(value:Boolean):void {
			_selectShow = value;
		}
		...
		public function set selectedIndex(value:int):void {
			if (_selectedIndex != value) {
				...
				if (_selectHandler != null) {
					_selectHandler.applyWith([value]);
				}
		...
		public function get renderHandler():Method {
			return _renderHandler;
		}
		
		public function set renderHandler(value:Method):void {
			_renderHandler = value;
		}
		
		/**单元格鼠标事件处理器(默认返回参数type:String,index:int)*/
		public function get mouseHandler():Method {
			return _mouseHandler;
		}
		
		public function set mouseHandler(value:Method):void {
			_mouseHandler = value;
		}
		...
		public function renderItemsNow():void {
		...
		protected function renderItem(cell:Box, index:int):void {
			...
			if (_renderHandler != null) {
				_renderHandler.applyWith([cell, index]);
			}
		...
		public function set array(value:Array):void {
			...
			_totalPage = Math.ceil(length / pageSize);
			//重设selectedIndex
			...
			if (_scrollBar) {
				...
				_scrollBar.visible = _scrollBarShowForever || _totalPage > 1;
...
//				_scrollBar.disabled = _totalPage <= 1;
//				if (!_scrollBar.disabled) {
//					_scrollBar.scrollSize = _cellSize;
//					_scrollBar.thumbPercent = lineNum / lineCount;
//					_scrollBar.setScroll(0, (lineCount - lineNum) * _cellSize, _startIndex * _cellSize);
//				} else {
...
//				}
		...
		public function get scrollBarShowForever():Boolean {
			return _scrollBarShowForever;
		}
		
		public function set scrollBarShowForever(value:Boolean):void {
			_scrollBarShowForever = value;
		}
		
		public function get renderShowForever():Boolean {
			return _renderShowForever;
		}
		
		public function set renderShowForever(value:Boolean):void {
			_renderShowForever = value;
		}
		...
		public function set page(value:int):void {
			...
			startIndex = _page * pageSize;
		}
		
		public function changePageToItem(item:Object):void {
		...
		public function autoScrollTo(index:int):void {
			startIndex = Math.max(0, Math.min(index, _array.length - cells.length));
			if (_scrollBar) {
				_scrollBar.value = index * _cellSize;
			}
		}
