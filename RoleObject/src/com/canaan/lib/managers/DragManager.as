package com.canaan.lib.managers
{
	...
	public class DragManager extends Sprite
	{
        ...
        public function doDrag(dragInitiator:DisplayObject, dragImage:DisplayObject = null, data:Object = null, lockCenter:Boolean = false, offsetX:int = 0, offsetY:int = 0):void {
			clearDrag();
			if (dragImage == null) {
			...
			startX = mouseX;
			startY = mouseY;
			TimerManager.getInstance().doFrameLoop(1, mouseMoveHandler);
//			StageManager.getInstance().registerHandler(MouseEvent.MOUSE_MOVE, mouseMoveHandler);
		...
		private function mouseMoveHandler():void {
			...
			if (startX != mouseX || startY != mouseY) {
				...
				startX = mouseX;
				startY = mouseY;
				dispatchEvent(new DragEvent(DragEvent.DRAG_MOVE, dragInitiator, data));
			}
		}
		...
		private function clearDrag():void {
			if (dragImage != null && dragInitiator != dragImage) {
				DisplayUtil.removeChild(dragImage.parent, dragImage);
			}
			TimerManager.getInstance().clear(mouseMoveHandler);
