...
package com.canaan.lib.animation
{
	...
	public class Tween
	{
		...
		public function reset(target:Object = null, time:Number = 0, transition:Object="linear"):Tween
		{
			mTarget = target;
			mCurrentTime = 0;
			mTotalTime = Math.max(0.0001, time / 1000);
		...
		{
			if (isNaN(time)) {
				time = 1.0 / StageManager.getInstance().fps;
			}
		...
		{
			var tween:Tween = sTweenPool.length ? sTweenPool.pop().reset(target, time, transition) :
		...
		public static function toPool(tween:Tween):void
		{
			...
			tween.mInPool = false;
			tween.stop();
			if (sTweenPool.indexOf(tween) == -1)
				sTweenPool.push(tween);
		}
		
		public function get inPool():Boolean { return mInPool; }
		public function set inPool(value:Boolean):void {mInPool = value};
		...
		public function stop():void
		{
			if (!running)
				return;
			TimerManager.getInstance().clear(advanceTime);
		}
		
		public function get running():Boolean
		{
			return TimerManager.getInstance().running(advanceTime);
		}
		...
		public function get transitionValue():Number
		{
			...
			return reversed ? mTransitionFunc(1.0 - ratio) : mTransitionFunc(ratio);
		}
	}
}
