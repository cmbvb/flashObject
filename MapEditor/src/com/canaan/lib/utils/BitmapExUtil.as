package com.canaan.lib.utils
{
	...
	public class BitmapExUtil
	{
		...
		/**
		 * 将bitmapData转换为BitmapDataEx
		 * @param source
		 * @param anchor
		 * @return 
		 * 
		 */		
		public static function createBitmapDataEx2(source:BitmapData, anchor:Point):BitmapDataEx {
			var x:int = -anchor.x;
			var y:int = -anchor.y;
			// 获取draw出的bitmapData的真实可视矩形
			var realRect:Rectangle = DisplayUtil.getVisualRect(source);
			// 如果真是可视矩形不为空
			if (!realRect.isEmpty()) {
				// 如果真实可视矩形bitmapData尺寸不一致则说明bitmapData中存在透明像素
				if ((source.width != realRect.width || source.height != realRect.height)) {
					// 将bitmapData中非透明像素拷贝到一个新的bitmapData中
					var rectBitmapData:BitmapData = new BitmapData(realRect.width, realRect.height, true, 0x00000000);
					var point:Point = DisplayUtil.gPoint;
					point.x = 0;
					point.y = 0;
					rectBitmapData.copyPixels(source, realRect, point);
					source.dispose();
					source = rectBitmapData;
					x += realRect.x;
					y += realRect.y;
				}
			} else {
				var emptyBitmapData:BitmapData = new BitmapData(1, 1, true, 0x00000000);
				source.dispose();
				source = emptyBitmapData;
			}
			
			return new BitmapDataEx(source, x, y);
		}
