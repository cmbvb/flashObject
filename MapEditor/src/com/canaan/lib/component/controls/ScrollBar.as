	...
	public class ScrollBar extends UIComponent {
		...
		protected function onSliderChange(e:Event):void {
			...
			if (_changeHandler != null) {
				_changeHandler.applyWith([value]);
			}
		}
		...
		protected function onButtonMouseDown(e:MouseEvent):void {
			...
			slide(isUp);
			TimerManager.getInstance().doOnce(Styles.scrollBarDelayTime, startLoop, [isUp]);
			StageManager.getInstance().stage.addEventListener(MouseEvent.MOUSE_UP, onStageMouseUp);
		}
		...
		public function justSetValue(value:Number):void {
			_slider.justSetValue(value);
		}
...
//			_slider.tick = value;
		...
		protected function onTargetMouseDown(e:MouseEvent):void {
			...
			TimerManager.getInstance().clear(tweenMove);
			if (!this.contains(e.target as DisplayObject)) {
				StageManager.getInstance().stage.addEventListener(MouseEvent.MOUSE_UP, onStageMouseUp2);
				StageManager.getInstance().stage.addEventListener(Event.ENTER_FRAME, onStageEnterFrame);
				_lastPoint = new Point(StageManager.getInstance().stage.mouseX, StageManager.getInstance().stage.mouseY);
			}
		}
		
		protected function onStageEnterFrame(e:Event):void {
			_lastOffset = _slider.direction == VERTICAL ? StageManager.getInstance().stage.mouseY - _lastPoint.y : StageManager.getInstance().stage.mouseX - _lastPoint.x;
			if (Math.abs(_lastOffset) >= 1) {
				_lastPoint.x = StageManager.getInstance().stage.mouseX;
				_lastPoint.y = StageManager.getInstance().stage.mouseY;
		...
		protected function onStageMouseUp2(e:MouseEvent):void {
			StageManager.getInstance().stage.removeEventListener(MouseEvent.MOUSE_UP, onStageMouseUp2);
			StageManager.getInstance().stage.removeEventListener(Event.ENTER_FRAME, onStageEnterFrame);
			if (Math.abs(_lastOffset) > 50) {
				_lastOffset = 50 * (_lastOffset > 0 ? 1 : -1);
			}
			TimerManager.getInstance().doFrameLoop(1, tweenMove);
		}
		...
		protected function onMouseWheel(e:MouseEvent):void {
			value += (e.delta < 0 ? 1 : -1) * _scrollSize;
